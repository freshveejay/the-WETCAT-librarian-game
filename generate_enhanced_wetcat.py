...
def process_sprite(url, output_path, target_size):
    ...
    for item in data:
        ...
        is_bg = any(
            all(abs(item[i] - bg[i]) < 30 for i in range(3)) for bg in bg_samples
        )
...
for sprite in sprites:
    ...
    if url:
        ...
        for path in [
            f"public/sprites/{sprite['name']}.png",
            f"src/assets/sprites/{sprite['name']}.png",
        ]:
...
for particle in particles:
    ...
    if url:
        for path in [
            f"public/sprites/{particle['name']}.png",
            f"src/assets/sprites/{particle['name']}.png",
        ]:
